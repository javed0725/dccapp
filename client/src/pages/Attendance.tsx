...
function SmartAttendanceExport({
  ...
  const downloadReport = async () => {
    ...
    setIsExporting(true);
    try {
      const response = await fetch("/api/attendance/smart/pdf", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ batchName, data }),
      });
      if (!response.ok) {
        const errorBody = await response.json().catch(() => null);
        throw new Error(errorBody?.message || "Failed to generate attendance PDF");
      }
