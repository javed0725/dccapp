...
// ── PaymentStatusModal helpers ────────────────────────────────────────────────

/** Strips non-digits and ensures the Bangladesh country code (88) is present. */
function normalizeBDPhone(raw: string): string {
  const digits = raw.replace(/\D/g, "");
  if (digits.startsWith("88")) return digits;   // already 8801XXXXXXXXX
  if (digits.startsWith("0"))  return "88" + digits; // 01XXXXXXXXX → 8801XXXXXXXXX
  return "88" + digits;                          // bare 1XXXXXXXXX → 881XXXXXXXXX
}

/** Bangla fee-reminder message — no amount, just name + month. */
function getReminderMessage(studentName: string, monthName: string): string {
  return `প্রিয় অভিভাবক, ডায়নামিক কোচিং সেন্টার থেকে জানানো যাচ্ছে যে, ${studentName}-এর ${monthName} মাসের কোচিং ফি বকেয়া রয়েছে। অনুগ্রহ করে দ্রুত পরিশোধ করার জন্য অনুরোধ করা হচ্ছে। ধন্যবাদ।`;
}

/** Minimal inline WhatsApp SVG — lucide-react has no WhatsApp icon. */
              ...
              >
                ...
                {s.mobileNumber && (
                  <div className="shrink-0 flex items-center gap-1">
                    ...
                    {/* SMS with pre-filled Bangla reminder */}
                    <a
                      href={`sms:${s.mobileNumber}?body=${encodeURIComponent(getReminderMessage(s.name, month))}`}
                      title={`SMS reminder to ${s.name}`}
                    ...
                    {/* WhatsApp with pre-filled Bangla reminder */}
                    <button
                      title={`WhatsApp reminder to ${s.name}`}
                      className="p-1.5 rounded-lg bg-[#dcfce7] dark:bg-[#14532d]/40 text-[#16a34a] dark:text-[#4ade80] hover:bg-[#bbf7d0] dark:hover:bg-[#14532d]/70 transition-colors"
                      onClick={e => {
                        e.stopPropagation();
                        const phone = normalizeBDPhone(s.mobileNumber);
                        const msg   = getReminderMessage(s.name, month);
